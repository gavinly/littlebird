defmodule HelloWorldWeb.IntentController do
  use HelloWorldWeb, :controller
  require Logger
  alias HelloWorld.IntentStore

  def index(conn, _params) do
    render(conn, :index, layout: {HelloWorldWeb.Layouts, :app})
  end
  ...
  def list(conn, _params) do
    all_intents = IntentStore.list_intents()
    pending_intents = Enum.filter(all_intents, fn intent -> intent.status == "pending" end)
    matched_intents = Enum.filter(all_intents, fn intent -> intent.status == "matched" end)
                     |> Enum.sort_by(fn intent -> intent.timestamp end, {:desc, DateTime})

    Logger.info("Listing pending intents: #{inspect(pending_intents)}")
    Logger.info("Listing matched intents: #{inspect(matched_intents)}")
